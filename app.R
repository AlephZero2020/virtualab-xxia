              ...
              div(
                ...
                fluidRow(
                  column(
                    ...
                    hr(),
                    downloadButton("desc_poblacion_marco", "Descargar población (CSV)", class = "btn-block")
                  ),
                  column(
                    8,
                    div(
              ...
              div(
                ...
                sidebarLayout(
                  sidebarPanel(
                    ...
                    downloadButton("vii_descargar_csv", "Descargar muestra (CSV)", class = "btn-block")
                  ),
...
server <- function(input, output, session) {
  ...
  output$desc_poblacion_marco <- downloadHandler(
    filename = function() paste0("poblacion_", Sys.Date(), ".csv"),
    content = function(file) {
      write.csv(marco_df(), file, row.names = FALSE, fileEncoding = "UTF-8")
    }
  )
  ...
  output$vii_descargar_csv <- downloadHandler(
    filename = function() paste0("muestra_vii_", Sys.Date(), ".csv"),
    content = function(file) {
      write.csv(muestra_export_vii(), file, row.names = FALSE, fileEncoding = "UTF-8")
    }
  )
